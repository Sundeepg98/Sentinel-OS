        ...
        <div className={cn("flex flex-col border-r border-white/5 transition-all duration-500", showCanvas ? "flex-1" : "flex-[2]")}>
          {status === 'idle' ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <p className="text-neutral-500 font-mono animate-pulse uppercase tracking-widest text-sm">Synthesizing Chaos...</p>
            </div>
          ) : (
            <>
              <div className="p-4 bg-rose-500/5 border-b border-rose-500/10">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-rose-400 text-sm uppercase tracking-wide">P0 ALERT: {incident?.title}</h4>
                ...
                <AnimatePresence>
                  {visibleLogs.map((log, i) => (
                    <motion.div 
                      ...
                      className={cn(
                        "mb-2 break-all",
                        log.toLowerCase().includes('error') || log.toLowerCase().includes('fatal') ? "text-rose-400" :
                        log.toLowerCase().includes('warn') ? "text-amber-400" : "text-emerald-400/70"
                      )}
                    >
                      <span className="text-neutral-600 mr-2">[{new Date().toISOString().split('T')[1].slice(0,-1)}]</span>
                      {log}
                    </motion.div>
                  ))}
                </AnimatePresence>
          ...
          {status === 'completed' && evaluation && (
            <div className="flex-1 flex flex-col p-6 overflow-y-auto animate-in slide-in-from-right-4 duration-500 h-full">
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="text-xs font-bold text-neutral-500 uppercase mb-2">Technical Feedback</h4>
                ...
                <div className="p-4 bg-white/[0.02] border border-white/5 rounded-lg">
                  <h4 className="text-xs font-bold text-emerald-500 uppercase mb-2">Actual Root Cause</h4>
