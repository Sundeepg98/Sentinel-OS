...
export const Diagnostics: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'stats' | 'knowledge' | 'ai-logs' | 'ui-logs'>(
    'stats'
  );
  ...
  const { data: stats } = useQuery<Stats>({
    queryKey: ['stats'],
    queryFn: () => fetchWithAuth<Stats>('/intelligence/stats', getToken),
    refetchInterval: 30000,
  });

  // ðŸ•µï¸ AI FAILURE LOGS QUERY
  const { data: aiLogs = [] } = useQuery<LogEntry[]>({
    queryKey: ['ai-logs'],
    queryFn: () => fetchWithAuth<LogEntry[]>('/admin/ai-logs', getToken),
    enabled: activeTab === 'ai-logs',
  });

  // ðŸ–¥ï¸ UI ERROR LOGS QUERY
  const { data: uiLogs = [] } = useQuery<LogEntry[]>({
    queryKey: ['ui-logs'],
    queryFn: () => fetchWithAuth<LogEntry[]>('/admin/ui-logs', getToken),
    enabled: activeTab === 'ui-logs',
  });

  // ðŸ“ FILE DELETION MUTATION
  const deleteMutation = useMutation({
    mutationFn: (filename: string) =>
      fetchWithAuth<{ success: boolean }>(`/admin/files/${dossier?.id}/${filename}`, getToken, {
        method: 'DELETE',
      }),
    onSuccess: () => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await fetchWithAuth(`/admin/upload/${dossier.id}`, getToken, {
        method: 'POST',
        body: formData,
      });
    ...
    >
      <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-white/[0.05] pb-6 gap-6">
        <div>
          ...
          <p className="text-neutral-500 text-xs mt-2 font-mono uppercase tracking-widest">
            Global Intelligence & Knowledge Management
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 bg-white/[0.02] p-1 rounded-xl border border-white/5">
          ...
          <TabButton active={activeTab === 'ui-logs'} onClick={() => setActiveTab('ui-logs')}>
            UI Crashes
          </TabButton>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-[#0d0d0d] border border-white/[0.05] rounded-2xl p-6 space-y-6 shadow-2xl">
                ...
                <div className="space-y-4">
                  ...
                  <ConfigItem
                    label="AI_CIRCUIT"
                    value={stats?.aiEngine?.circuitState}
                    color={
                      stats?.aiEngine?.circuitState === 'CLOSED'
                        ? 'text-emerald-400'
                        : 'text-rose-400'
                    }
                  />
              ...
              <div className="bg-[#0d0d0d] border border-white/[0.05] rounded-2xl p-6 space-y-6 shadow-2xl">
                ...
                <div className="space-y-4 font-mono text-[10px] text-neutral-600">
                  <StatusLine
                    ok={!stats?.isSyncing}
                    label="RAG WORKER"
                    detail={stats?.isSyncing ? 'SYNCHRONIZING' : 'IDLE'}
                  />
          ...
          >
                ...
                <button
                  onClick={async () => {
                    ...
                    if (name) {
                      ...
                      await fetchWithAuth(`/admin/companies/${cleanName}`, getToken, {
                        method: 'POST',
                      });
            ...
            <div className="grid grid-cols-1 gap-3">
              {dossier?.modules.map((mod) => (
                <div
                  key={mod.fullId}
                  className="bg-[#0d0d0d] border border-white/[0.05] p-4 rounded-xl flex items-center justify-between group hover:border-indigo-500/30 transition-all"
                >
                  <div className="flex items-center gap-4">
                  ...
                  <button
                    onClick={() =>
                      confirm(`Permanently delete ${mod.label}?`) &&
        ...
        {activeTab === 'ai-logs' && (
          ...
          >
            <Header
              label="AI Failure Logs"
              sub="System Exceptions & Prompt Errors"
        ...
        {activeTab === 'ui-logs' && (
          ...
          >
            <Header
              label="UI Error Telemetry"
              sub="Recorded Frontend Crashes & Stack Traces"
...
const TabButton = ({
  ...
  <button
    ...
    className={cn(
      ...
      active ? 'bg-white/10 text-white shadow-lg' : 'text-neutral-500 hover:text-neutral-300'
    )}
  >
    {children}
  </button>
...
const StatusLine = ({
  ...
  <div className="flex gap-3">
    <span className={cn(ok ? 'text-emerald-500' : warn ? 'text-amber-500' : 'text-rose-500')}>
      [{ok ? 'OK' : warn ? 'WARN' : 'BUSY'}]
    </span>
    <span>
      {label} {detail && `- ${detail}`}
    </span>
  ...
  <div className="space-y-4">
    {logs.length === 0 ? (
      ...
      logs.map((log, i) => (
        <div
          key={i}
          className="bg-[#0d0d0d] border border-white/[0.05] p-6 rounded-xl space-y-4 shadow-xl"
        >
          <div className="flex justify-between items-start">
            <span
              className={cn(
                'px-2 py-1 text-[10px] font-bold rounded uppercase tracking-tighter',
                type === 'AI' ? 'bg-rose-500/10 text-rose-500' : 'bg-amber-500/10 text-amber-500'
              )}
            >
              {type} FAILURE
            </span>
          ...
          <div>
            ...
            {log.stack && (
              <pre className="mt-2 bg-black/20 p-4 rounded-lg text-[10px] text-neutral-600 font-mono overflow-x-auto whitespace-pre">
                {log.stack}
              </pre>
            )}
          </div>
...
const StatCard = ({
  ...
  <div className="bg-[#0d0d0d] border border-white/[0.05] p-6 rounded-2xl shadow-xl space-y-4 hover:border-white/10 transition-all group">
    ...
    <div>
      ...
      <div className="text-[10px] text-neutral-600 uppercase tracking-widest mt-1 font-semibold">
        {sub}
      </div>
    </div>
  </div>
