...
export const CodeBlock: React.FC<CodeBlockProps> = ({
  ...
  return (
    <div
      className={cn(
        'bg-[#0a0a0a] rounded-xl overflow-hidden border border-white/[0.08] shadow-2xl my-4',
        className
      )}
    >
      ...
      <div
        ...
        tabIndex={0}
        role="region"
        aria-label={`Code implementation: ${title}`}
      >
        <pre className="text-[13px] font-mono leading-relaxed text-neutral-300">
          {code.split('\n').map((line, i) => {
            ...
            const tokenRegex =
              /(\/\/.*|--.*|#.*)|('.*?'|".*?"|`.*?`)|\b(const|let|await|async|function|return|if|else|local|import|export|from|class|extends|public|readonly|static)\b|\b(redis|consumer|kafka|process|Math|SELECT|UPDATE|SET|WHERE|FOR|BEGIN|COMMIT|pulumi|aws|awsx|std)\b/g;
            ...
            return (
              <div
                key={i}
                className="flex hover:bg-white/[0.02] transition-colors rounded px-2 -mx-2"
              >
                <span className="w-6 shrink-0 text-neutral-600 select-none border-r border-white/[0.05] mr-4 text-right pr-3">
                  {i + 1}
                </span>
