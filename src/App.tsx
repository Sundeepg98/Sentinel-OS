        ...
        <header className="h-16 shrink-0 px-8 flex justify-between items-center bg-black/40 backdrop-blur-md border-b border-white/[0.05] z-30">
          <div className="flex items-center gap-4">
            <select 
              ...
              onChange={(e) => { dossierData.setCompany(e.target.value); resetViews(); }}
            ...
            >
              {dossierData.allCompanies.map(c => <option key={c.id} value={c.id}>{c.name} Profile</option>)}
            </select>

            <button onClick={() => setIsGraphOpen(true)} className="p-2 bg-white/[0.03] border border-white/[0.08] rounded-lg text-neutral-400 hover:text-cyan-400 hover:border-cyan-500/30 transition-all shadow-sm" title="Open Knowledge Graph">
              <Network size={16} />
            </button>
            ...
            <button onClick={() => { setArenaMode(!arenaMode); setWarRoomMode(false); setDiagnosticsMode(false); }} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border shadow-sm", arenaMode ? "bg-indigo-500 border-indigo-400 text-white shadow-[0_0_20px_rgba(99,102,241,0.3)]" : "bg-white/[0.03] border-white/[0.08] text-neutral-400 hover:text-white")}>
              <Swords size={14} /> Arena {pinnedIds.length > 0 && `(${pinnedIds.length})`}
            </button>
            
            <button onClick={() => { setWarRoomMode(!warRoomMode); setArenaMode(false); setDiagnosticsMode(false); }} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border shadow-sm", warRoomMode ? "bg-rose-600 border-rose-500 text-white shadow-[0_0_20px_rgba(225,29,72,0.3)]" : "bg-white/[0.03] border-white/[0.08] text-neutral-400 hover:text-rose-400 hover:border-rose-500/30")}>
              <Terminal size={14} /> War Room
            </button>
          </div>

          <div className="flex items-center gap-4">
            <button onClick={() => window.open('/api/v1/portfolio/export', '_blank')} className="text-xs font-semibold text-neutral-400 hover:text-white transition-colors uppercase tracking-widest px-3 py-1.5 border border-white/5 hover:border-white/20 rounded-lg bg-white/[0.02]">Export Portfolio</button>
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          <ErrorBoundary>
            {dossierData.loading && !dossierData.dossier ? (
              ...
              <div className="flex-1 overflow-y-auto"><Diagnostics /></div>
            ) : arenaMode ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center opacity-50 p-20 text-center h-full">
                ...
                <button onClick={() => setDiagnosticsMode(true)} className="mt-6 px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-bold uppercase tracking-widest transition-colors">Open System Control</button>
              </div>
            ) : (
